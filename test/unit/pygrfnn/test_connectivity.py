...
# @Last Modified time: 2015-05-07 00:05:06
...
def test_connection_same_size_layers():
    C = make_connections(layer_N50_narrow, layer_N50_wide, 1, 1.05)
    assert C.shape == (len(layer_N50_wide.f), len(layer_N50_narrow.f))

def test_connection_different_size_layers():
    C = make_connections(layer_N50_narrow, layer_N100_narrow, 1, 1.05)
...
def test_frequency_relationships():
    ...
    C = make_connections(l1, l2, 1, 1.05)
    model = Model()
    model.add_layer(l1)
    model.add_layer(l2)
    conn = model.connect_layers(l1, l2, C, '2freq')
