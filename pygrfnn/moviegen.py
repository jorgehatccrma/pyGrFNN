...
def animate_tf_detail(TF,
    ...
    # to animate
    (fig, tf_line, t_line, detail) = tf_detail(TF,
                                               t_ods,
                                               T,
                                               title,
                                               np.max(t_ods) / 2.0,
    ...
    def animate(i):
        ...
        print('  {0:.2f}/{1:.2f}'.format(t_detail, max(t_ods)),
              end='\r')
        sys.stdout.flush()
    ...
    with TmpFile(suffix='.mp4') as temp_video, \
        ...
        write(temp_audio.name, signal.sr, signal)
